...
@Startup
@Singleton
public class ResultConsumerSetup {
	...
	@Inject
    @ConfigProperty(name = "kafka.resultGroupId", defaultValue = "consumerResultGroupId")
    ...
    private void init() {
        ...
        props.setProperty(ConsumerConfig.KEY_DESERIALIZER_CLASS_CONFIG, ResultDeserializer.class.getName());
        props.setProperty(ConsumerConfig.VALUE_DESERIALIZER_CLASS_CONFIG, ResultDeserializer.class.getName());
        props.setProperty(ConsumerConfig.ENABLE_AUTO_COMMIT_CONFIG, "false");
        props.setProperty(ConsumerConfig.GROUP_ID_CONFIG, resultGroup);
        ...
        for (int i = 0; i < Integer.parseInt(instances); i++) {
        	logger.fine("Creating new Result consumer thread");
        	mes.execute(new ResultConsumer(props, resultTopic));
        }
