...
public class ResultConsumer implements Runnable {
	...
	private KafkaConsumer<String, Result> consumer;
	private AtomicBoolean isClosed = new AtomicBoolean();
    private Logger logger = Logger.getLogger(this.getClass().getCanonicalName());

	public ResultConsumer (Properties properties, String... topics) {
	...
	public void run() {
		...
		try {
			while (!isClosed.get()) {
				...
				ConsumerRecords<String, Result> records = consumer.poll(Duration.of(30000, ChronoUnit.MILLIS));
				for (ConsumerRecord<String, Result> record : records) {
					logger.info("RUN: has record");
					System.out.println(record.value().getUrl() + " : " + Thread.currentThread().getName());
				}
